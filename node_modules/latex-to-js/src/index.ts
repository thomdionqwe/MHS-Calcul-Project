...
interface Options {
    LatexAST: boolean;
    functionPrefix?: boolean|string;
    constantPrefix?: boolean|string;
    functions: {
        int: string;
        sum: string;
        sqrt: string;
...
function latex2js(latex: string, options?: Partial<Options>) {
    function err(errorHandler: ErrorHandler): Error {
    ...
    function Parser(tokens: any[], _options: Options) {
        const parsed = [], JS = [];
        let i = 0, _sub: boolean = false, _sum: boolean = false, _sumvar: string;
        const nextToken = () => {
            i += 1;
            return tokens[i]
        }
        function eat(token: any) {
            if (token.type === "command") {
                ...
                let j = i + 1;
                JS.push(token.text)
                if (_sum) _sumvar = token.text;
                if (tokens[j] && tokens[j].type === "any" && !_sub) JS.push("*")
                return token.text;
